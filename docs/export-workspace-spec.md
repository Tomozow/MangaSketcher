...
- 既存仕様との関係: `docs/web-implementation-spec.md` §1.2 の非ゴール「書き出し（PNG/PDF/ZIP/共有シート）」を、**本機能（ワークスペース ZIP の合成・内部構造・共有 2 段階）に限り**上書きする。形式選択・PDF・ページ部分集合・生成中キャンセルは `docs/export-formats-spec.md` が上書きする。サーバ送信は依然非ゴール
...
| 非目標 | 固定理由 |
| --- | --- |
| ページ選択書き出し | **上書き済み。** 部分集合は `docs/export-formats-spec.md` §2.3 |
...
| JPEG をダウンロード形式として出す | **一部上書き。** 端末へ渡す JPEG ファイルは出さない。PDF 内埋め込み JPEG は `docs/export-formats-spec.md` §6.3 |
...
生成中の「キャンセル」ボタンは `docs/export-formats-spec.md` §2.4 が定める。PNG ZIP 単独だった初回スコープの「置かない」は廃止する。
