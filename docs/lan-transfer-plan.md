...
- **期限なし。** 受け取り号は一覧表示中同じ 6 桁のまま。ZIP の成功 GET は zip だけ消し、号（メタ）は残す。同一号の並行 PUT は禁止（zip があるあいだ 409）
...
| 受け取る | ボタンなし。一覧表示中かつハブ生存なら 6 桁を自動表示 |
